...
import {DEPLOY_SEARCH, DEPLOY_SEARCH_OK, LOAD_KNOWN_SERVERS, LOAD_KNOWN_SERVERS_OK, LOAD_SINGLE_SERVER, LOAD_SINGLE_SERVER_OK, RELEASE_SERVER, RESERVE_SERVER, UPDATE_DEPLOY_SEARCH} from './server.actions';
...
export class ServersEffects {
  ...
  RESERVE_SERVER$ = createEffect(() => this.actions$.pipe(
    ...
    switchMap((action) => this.http.post<IReservation>(
      ...
      map((reservation: IReservation) => LOAD_SINGLE_SERVER({serverName: action.event.serverName})),
      catchError(errorResponse => of(backendErrorOccurred({errorResponse})))
    ))
  ));
  ...
  RELEASE_SERVER$ = createEffect(() => this.actions$.pipe(
    ...
    exhaustMap((action) => this.http.delete(`/api/v1/servers/${action.serverName}/reservation`).pipe(
      map(() => LOAD_SINGLE_SERVER({serverName: action.serverName})),
      catchError(errorResponse => of(backendErrorOccurred({errorResponse})))
    ))
  ));
